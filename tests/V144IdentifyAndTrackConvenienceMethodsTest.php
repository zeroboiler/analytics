...
// ── Version Consistency (v145.0.0) ────────────────────────────────────

describe('V144 Version Consistency', function () {
    it('has VERSION 145.0.0 in AnalyticsEvent', function () {
        expect(AnalyticsEvent::VERSION)->toBe('145.0.0');
    });
...
// ── identifyAndTrack Method (v145.0.0) ───────────────────────────────
...
// ── Web Vitals Convenience Method (v145.0.0) ─────────────────────────
...
// ── New Engagement Convenience Methods (v145.0.0) ───────────────────
...
// ── New SaaS Lifecycle Convenience Methods (v145.0.0) ────────────────
...
// ── Event Catalog Completeness (v145.0.0) ───────────────────────────
...
// ── Version Sweep Integrity (v145.0.0) ──────────────────────────────

describe('V144 Version Sweep Integrity', function () {
    it('composer.json has version 145.0.0', function () {
        $composer = json_decode(file_get_contents(__DIR__ . '/../../composer.json'), true);
        expect($composer['version'])->toBe('145.0.0');
    });

    it('package.json has version 145.0.0', function () {
        $pkg = json_decode(file_get_contents(__DIR__ . '/../../package.json'), true);
        expect($pkg['version'])->toBe('145.0.0');
    });

    it('JS client getVersion returns 145.0.0', function () {
        $content = file_get_contents(__DIR__ . '/../../resources/js/analytics.js');
        expect($content)->toContain("return '145.0.0'");
    });
