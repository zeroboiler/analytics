...
/**
 * Create a ConfigRepository mock that returns [] for the daily_health_report
 * config section (so constructor doesn't emit array-offset-on-bool warnings)
 * and false for all other config reads (all providers disabled).
 */
function mockConfigReturningFalse(): ConfigRepository
{
    $config = mock(ConfigRepository::class);
    $config->shouldReceive('get')->andReturnUsing(
        fn (string $key, mixed $default = null): mixed => str_starts_with($key, 'zeroboiler.analytics.daily_health_report')
            ? $default ?? []
            : false,
    );
...
test('generate method returns complete report structure', function (): void {
    ...
    $config = mockConfigReturningFalse();

    $report = (new AnalyticsDailyHealthReportService($cache, $config))->generate(true);

    expect($report)->toBeArray()
        ->toHaveKeys([
            'generated_at', 'overall_score', 'grade', 'domains',
            'critical_issues', 'warnings', 'recommendations', 'metadata',
        ]);
...
test('score returns integer', function (): void {
    ...
    $score = (new AnalyticsDailyHealthReportService($cache, $config))->score();
...
test('status returns valid status string', function (): void {
    ...
    $status = (new AnalyticsDailyHealthReportService($cache, $config))->status();
...
test('criticalIssues returns array with correct structure', function (): void {
    ...
    $issues = (new AnalyticsDailyHealthReportService($cache, $config))->criticalIssues();
...
test('domainScore returns score and status for valid domain', function (): void {
    ...
    $result = (new AnalyticsDailyHealthReportService($cache, $config))->domainScore('provider_health');
...
test('domainScore returns unknown for invalid domain', function (): void {
    ...
    $result = (new AnalyticsDailyHealthReportService($cache, $config))->domainScore('nonexistent_domain');
...
test('clearCache removes cached report', function (): void {
    ...
    (new AnalyticsDailyHealthReportService($cache, $config))->clearCache();
})->throwsNoExceptions();
...
test('cache is used when available and forceRefresh is false', function (): void {
    ...
    $report = (new AnalyticsDailyHealthReportService($cache, $config))->generate(false);
...
test('metadata has required keys', function (): void {
    ...
    expect($report['metadata']['version'])->toBe('118.0.0');
