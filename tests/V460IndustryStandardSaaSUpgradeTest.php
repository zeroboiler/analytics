...
/**
 * V5.9.0 — Industry-Standard SaaS Analytics Upgrade Test
 *
 * Validates:
 * 1. Version consistency across all source files (5.9.0)
...
test('v5.9.0: AnalyticsEvent VERSION is 5.9.0', function (): void {
    expect(AnalyticsEvent::VERSION)->toBe('5.9.0');
});

test('v5.9.0: composer.json version is 5.9.0', function (): void {
    $composer = json_decode(file_get_contents(__DIR__ . '/../composer.json'), true);
    expect($composer['version'])->toBe('5.9.0');
});

test('v5.9.0: JS client version is 5.9.0', function (): void {
    $js = file_get_contents(__DIR__ . '/../resources/js/analytics.js');
    expect($js)->toContain("'5.9.0'");
    expect($js)->toContain('@version 5.9.0');
});

test('v5.9.0: Svelte composables version is 5.9.0', function (): void {
    $svelte = file_get_contents(__DIR__ . '/../resources/js/useAnalytics.svelte.js');
    expect($svelte)->toContain('@version 5.9.0');
});

test('v5.9.0: TypeScript definitions version is 5.9.0', function (): void {
    $ts = file_get_contents(__DIR__ . '/../resources/js/analytics.d.ts');
    expect($ts)->toContain('@version 5.9.0');
});

test('v5.9.0: no stale @version docblocks remain', function (): void {
    ...
    foreach ($lines as $line) {
        if (! str_contains($line, '@version 5.9.0')) {
            $stale[] = $line;
        }
    }
...
test('v5.9.0: no stale VERSION constants remain', function (): void {
    $output = shell_exec('grep -rn "VERSION = '"'"'[0-9]' . escapeshellarg(__DIR__ . '/../src') . ' --include="*.php" 2>/dev/null') ?: '';
    $lines = array_filter(explode("\n", trim($output)));
...
test('v5.9.0: AnalyticsAIService class exists', function (): void {
    expect(class_exists(AnalyticsAIService::class))->toBeTrue();
});

test('v5.9.0: AnalyticsAIService has required methods', function (): void {
...
test('v5.9.0: AnalyticsAIService is final', function (): void {
    $ref = new ReflectionClass(AnalyticsAIService::class);
    expect($ref->isFinal())->toBeTrue();
});

test('v5.9.0: AnalyticsAIService constructor has proper type hints', function (): void {
    $ref = new ReflectionClass(AnalyticsAIService::class);
...
test('v5.9.0: AnalyticsAIService has return type declarations', function (): void {
    $ref = new ReflectionClass(AnalyticsAIService::class);
...
test('v5.9.0: AnalyticsAIService detectAnomaly returns null for disabled service', function (): void {
...
test('v5.9.0: AnalyticsAIService detectAnomaly returns null for insufficient data', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
...
test('v5.9.0: AnalyticsAIService detectAnomaly detects anomaly after sufficient data', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
...
test('v5.9.0: AnalyticsAIService generateInsights returns insights for event data', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.ai', [])->andReturn(['enabled' => true]);
...
test('v5.9.0: AnalyticsAIService generateInsights detects missing lifecycle events', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.ai', [])->andReturn(['enabled' => true]);
...
test('v5.9.0: AnalyticsAIService analyzeTrend returns flat for insufficient data', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.ai', [])->andReturn(['enabled' => true]);
...
test('v5.9.0: AnalyticsAIService analyzeTrend detects upward trend', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.ai', [])->andReturn(['enabled' => true]);
...
test('v5.9.0: AnalyticsAIService analyzeTrend detects downward trend', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.ai', [])->andReturn(['enabled' => true]);
...
test('v5.9.0: AnalyticsAIService suggestEvents returns catalog coverage', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.ai', [])->andReturn(['enabled' => true]);
...
test('v5.9.0: AnalyticsAIService getStatus returns config', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.ai', [])->andReturn([
        'enabled' => true,
...
test('v5.9.0: AnalyticsAIService clearBuffer works', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.ai', [])->andReturn(['enabled' => true]);
...
test('v5.9.0: EventExperimentTracker class exists', function (): void {
    expect(class_exists(EventExperimentTracker::class))->toBeTrue();
});

test('v5.9.0: EventExperimentTracker is final', function (): void {
    $ref = new ReflectionClass(EventExperimentTracker::class);
    expect($ref->isFinal())->toBeTrue();
});

test('v5.9.0: EventExperimentTracker has required methods', function (): void {
    $ref = new ReflectionClass(EventExperimentTracker::class);
    $methods = array_map(fn (\ReflectionMethod $m) => $m->getName(), $ref->getMethods(\ReflectionMethod::IS_PUBLIC));
...
test('v5.9.0: EventExperimentTracker has return type declarations', function (): void {
    $ref = new ReflectionClass(EventExperimentTracker::class);
...
test('v5.9.0: EventExperimentTracker constructor has proper type hints', function (): void {
    $ref = new ReflectionClass(EventExperimentTracker::class);
    $ctor = $ref->getMethod('__construct');
    $params = $ctor->getParameters();
...
test('v5.9.0: EventExperimentTracker creates experiment correctly', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
...
test('v5.9.0: EventExperimentTracker tracks events and updates conversion rates', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.experiment', [])->andReturn(['enabled' => true]);
...
test('v5.9.0: EventExperimentTracker significance calculation returns correct structure', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
...
test('v5.9.0: EventExperimentTracker pause/resume lifecycle works', function (): void {
    $config = Mockery::mock(\Illuminate\Contracts\Config\Repository::class);
    $cache = Mockery::mock(\Illuminate\Contracts\Cache\Repository::class);
    $config->shouldReceive('get')->with('zeroboiler.analytics.experiment', [])->andReturn(['enabled' => true]);
    $cache->shouldReceive('put')->andReturnTrue();
...
test('v5.9.0: SaaSQuickStartService class exists', function (): void {
    expect(class_exists(SaaSQuickStartService::class))->toBeTrue();
});

test('v5.9.0: SaaSQuickStartService is final', function (): void {
    $ref = new ReflectionClass(SaaSQuickStartService::class);
    expect($ref->isFinal())->toBeTrue();
});

test('v5.9.0: SaaSQuickStartService has all tracking methods', function (): void {
    $ref = new ReflectionClass(SaaSQuickStartService::class);
    $methods = array_map(fn (\ReflectionMethod $m) => $m->getName(), $ref->getMethods(\ReflectionMethod::IS_PUBLIC));
...
test('v5.9.0: SaaSQuickStartService methods have return type void', function (): void {
    $ref = new ReflectionClass(SaaSQuickStartService::class);
...
test('v5.9.0: SaaSQuickStartService constructor accepts AnalyticsManager', function (): void {
    $ref = new ReflectionClass(SaaSQuickStartService::class);
    $ctor = $ref->getMethod('__construct');
    $params = $ctor->getParameters();
...
test('v5.9.0: config has ai section', function (): void {
...
test('v5.9.0: config has experiment section', function (): void {
    $config = file_get_contents(__DIR__ . '/../config/zeroboiler.php');
    expect($config)->not->toBeFalse();
...
test('v5.9.0: config has 30+ sections total', function (): void {
    $config = file_get_contents(__DIR__ . '/../config/zeroboiler.php');
...
test('v5.9.0: ServiceProvider registers AnalyticsAIService', function (): void {
...
test('v5.9.0: ServiceProvider registers EventExperimentTracker', function (): void {
    $provider = file_get_contents(__DIR__ . '/../src/AnalyticsServiceProvider.php');
    expect($provider)->toContain('EventExperimentTracker::class');
    expect($provider)->toContain('singleton(EventExperimentTracker');
});

test('v5.9.0: ServiceProvider registers SaaSQuickStartService', function (): void {
    $provider = file_get_contents(__DIR__ . '/../src/AnalyticsServiceProvider.php');
    expect($provider)->toContain('SaaSQuickStartService::class');
    expect($provider)->toContain('singleton(SaaSQuickStartService');
});

test('v5.9.0: ServiceProvider has import statements for new services', function (): void {
    $provider = file_get_contents(__DIR__ . '/../src/AnalyticsServiceProvider.php');
...
test('v5.9.0: README version badge is 5.9.0', function (): void {
    $readme = file_get_contents(__DIR__ . '/../README.md');
    expect($readme)->toContain('version-5.9.0-blue');
});

// ── CHANGELOG ────────────────────────────────────────────────────────

test('v5.9.0: CHANGELOG has 5.9.0 entry', function (): void {
    $changelog = file_get_contents(__DIR__ . '/../CHANGELOG.md');
    expect($changelog)->toContain('## [5.9.0]');
});
