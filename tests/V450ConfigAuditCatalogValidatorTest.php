...
test('feature 13: version consistency — AnalyticsEvent::VERSION is 5.0.0', function (): void {
...
test('feature 18: JS client version is synced to 4.5.0', function (): void {
    ...
    expect($jsContent)->toContain("return '5.0.0'");

    // Header version is correct
    expect($jsContent)->toContain('@version 5.0.0');
});
...
test('feature 19: TypeScript definitions version is 4.5.0', function (): void {
    ...
    expect($dtsContent)->toContain('@version 5.0.0');
});
...
test('feature 20: Svelte composables version is 4.5.0', function (): void {
    ...
    expect($svelteContent)->toContain('@version 5.0.0');
});
...
test('feature 22: ServiceProvider docblock version is 4.5.0', function (): void {
    ...
    expect($providerContent)->toContain('@version 5.0.0');
});
...
test('feature 24: new service files exist with strict types and proper namespace', function (): void {
    ...
    expect($auditContent)->toContain('@version 5.0.0');
    expect($validatorContent)->toContain('@version 5.0.0');
});

test('feature 25: README documents v5.0.0 features', function (): void {
    $readme = file_get_contents(__DIR__ . '/../README.md');
    $readmeContent = is_string($readme) ? $readme : '';

    expect($readmeContent)->toContain("What's New in v5.0.0");
    ...
    expect($readmeContent)->toContain('version-5.0.0');
});
