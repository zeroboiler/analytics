...
test('feature 13: version consistency — AnalyticsEvent::VERSION is 4.5.0', function (): void {
    expect(AnalyticsEvent::VERSION)->toBe('4.5.0');
});
...
test('feature 18: JS client version is synced to 4.5.0', function (): void {
    ...
    expect($jsContent)->toContain("return '4.5.0'");

    // Header version is correct
    expect($jsContent)->toContain('@version 4.5.0');
});
...
test('feature 19: TypeScript definitions version is 4.5.0', function (): void {
    ...
    expect($dtsContent)->toContain('@version 4.5.0');
});
...
test('feature 20: Svelte composables version is 4.5.0', function (): void {
    ...
    expect($svelteContent)->toContain('@version 4.5.0');
});

test('feature 21: composer.json version is 4.5.0', function (): void {
    $composer = json_decode(file_get_contents(__DIR__ . '/../composer.json'), true);
    expect($composer)->not->toBeNull();
    expect($composer['version'])->toBe('4.5.0');
});
...
test('feature 22: ServiceProvider docblock version is 4.5.0', function (): void {
    ...
    expect($providerContent)->toContain('@version 4.5.0');
});
...
test('feature 24: new service files exist with strict types and proper namespace', function (): void {
    ...
    expect($auditContent)->toContain('@version 4.5.0');
    expect($validatorContent)->toContain('@version 4.5.0');
});

test('feature 25: README documents v4.5.0 features', function (): void {
    $readme = file_get_contents(__DIR__ . '/../README.md');
    $readmeContent = is_string($readme) ? $readme : '';

    expect($readmeContent)->toContain("What's New in v4.5.0");
    ...
    expect($readmeContent)->toContain('version-4.5.0');
});
