...
test('v8.6.0 — version consistency across all entry points', function (): void {
    ...
    expect(AnalyticsEvent::VERSION)->toBe('8.6.0');

    // composer.json
    $composer = json_decode(file_get_contents(base_path('composer.json')), true);
    expect($composer['version'])->toBe('8.6.0');

    // package.json
    $package = json_decode(file_get_contents(base_path('package.json')), true);
    expect($package['version'])->toBe('8.6.0');

    // JS client header
    $jsContent = file_get_contents(base_path('resources/js/analytics.js'));
    expect($jsContent)->toContain('@version 8.6.0');

    // JS getVersion()
    expect($jsContent)->toContain("return '8.6.0'");

    // TypeScript definitions
    $tsContent = file_get_contents(base_path('resources/js/analytics.d.ts'));
    expect($tsContent)->toContain('@version 8.6.0');

    // Svelte composable
    $svelteContent = file_get_contents(base_path('resources/js/useAnalytics.svelte.js'));
    expect($svelteContent)->toContain('@version 8.6.0');
});
