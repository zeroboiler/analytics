...
describe('EventAliasResolver — v2.49 alias management', function (): void {
    ...
    it('returns a valid summary', function (): void {
        ...
        expect($summary['version'])->toBe('2.49.0');
...
describe('EventCacheService — v2.49 core', function (): void {
    ...
    it('returns valid stats', function (): void {
        ...
        expect($stats['version'])->toBe('2.49.0');
...
describe('Version — v2.49 consistency', function (): void {
    it('manager version is 2.49.0', function (): void {
        expect($this->manager->version())->toBe('2.49.0');
    });

    it('composer version is 2.49.0', function (): void {
        $composer = json_decode(file_get_contents(__DIR__ . '/../composer.json'), true);

        expect($composer['version'])->toBe('2.49.0');
    });

    it('JS client version is 2.49.0', function (): void {
        $js = file_get_contents(__DIR__ . '/../resources/js/analytics.js');

        expect($js)->toContain("'2.49.0'");
    });

    it('TypeScript definitions version is 2.49.0', function (): void {
        $dts = file_get_contents(__DIR__ . '/../resources/js/analytics.d.ts');

        expect($dts)->toContain('2.49.0');
    });

    it('EventSourceTagger version is 2.49.0', function (): void {
        $tagger = file_get_contents(__DIR__ . '/../src/Services/EventSourceTagger.php');

        expect($tagger)->toContain("'2.49.0'");
    });

    it('EventForwardingService version is 2.49.0', function (): void {
        $forwarding = file_get_contents(__DIR__ . '/../src/Services/EventForwardingService.php');

        expect($forwarding)->toContain("'2.49.0'");
    });

    it('AnalyticsEventRouter version is 2.49.0', function (): void {
        $router = file_get_contents(__DIR__ . '/../src/Services/AnalyticsEventRouter.php');

        expect($router)->toContain("'2.49.0'");
    });

    it('controller version strings are 2.49.0', function (): void {
        $controller = file_get_contents(__DIR__ . '/../src/Http/Controllers/AnalyticsEventController.php');

        expect($controller)->toContain("'2.49.0'");
        expect($controller)->not->toContain("'2.47.0'");
        expect($controller)->not->toContain("'2.48.0'");
    });
});
