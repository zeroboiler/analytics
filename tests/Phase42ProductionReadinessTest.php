...
describe('Phase 42 — Post-v174-v177 Production Readiness', function (): void {
    ...
    it('composer.json version is 180.0.0', function (): void {
        $composer = json_decode(
            file_get_contents(dirname(__DIR__, 2) . '/composer.json'),
            true,
        );
        expect($composer['version'])->toBe('180.0.0');
    });

    it('package.json version is 180.0.0', function (): void {
        $pkg = json_decode(
            file_get_contents(dirname(__DIR__, 2) . '/package.json'),
            true,
        );
        expect($pkg['version'])->toBe('180.0.0');
    });

    it('AnalyticsEvent::VERSION is 180.0.0', function (): void {
        expect(AnalyticsEvent::VERSION)->toBe('180.0.0');
    });

    it('AnalyticsIntegrityCommand::EXPECTED_VERSION is 180.0.0', function (): void {
        ...
        expect($const)->toBe('180.0.0');
    });

    it('ServiceProvider @version is 180.0.0', function (): void {
        $doc = (new ReflectionClass(AnalyticsServiceProvider::class))->getDocComment();
        expect($doc)->not->toBeFalse();
        expect($doc)->toContain('@version 180.0.0');
    });

    it('README version badge matches', function (): void {
        $readme = file_get_contents(dirname(__DIR__, 2) . '/README.md');
        expect($readme)->toContain('version-180.0.0');
    });

    it('analytics.js header + getVersion returns 180.0.0', function (): void {
        $js = file_get_contents(dirname(__DIR__, 2) . '/resources/js/analytics.js');
        expect($js)->toContain('@version 180.0.0');
        expect($js)->toContain("'180.0.0'");
    });
