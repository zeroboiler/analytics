...
test('version consistency: all components report v5.3.0', function (): void {
    expect(AnalyticsEvent::VERSION)->toBe('5.3.0');

    // JS client version
    $jsClient = file_get_contents(__DIR__ . '/../resources/js/analytics.js');
    expect($jsClient)->toContain('@version 5.3.0');

    // Svelte composables version
    $composables = file_get_contents(__DIR__ . '/../resources/js/useAnalytics.svelte.js');
    expect($composables)->toContain('@version 5.3.0');
...
test('analytics event DTO is immutable and readonly', function (): void {
    ...
    expect($event->VERSION)->toBe('5.3.0');
});
