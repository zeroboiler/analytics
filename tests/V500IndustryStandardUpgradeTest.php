...
/**
 * V5.9.0 — Industry Standard SaaS Analytics Maturity Test.
 *
 * Validates v5.9.0 features: AnalyticsDataService (DAU/MAU, revenue trends,
 ...
 */
test('v5.9.0: version is 5.9.0 everywhere', function (): void {
...
test('v5.9.0: composer.json requires PHP 8.5+ and Laravel 13', function (): void {
    $composer = json_decode(file_get_contents(__DIR__ . '/../composer.json'), true);
...
test('v5.9.0: AnalyticsDataService class exists with required methods', function (): void {
...
test('v5.9.0: AnalyticsDataService DAU/MAU structure', function (): void {
    $cache = app('cache');
...
test('v5.9.0: AnalyticsDataService dashboard summary structure', function (): void {
    $cache = app('cache');
    $metrics = new AnalyticsMetrics;
...
test('v5.9.0: AnalyticsDataService revenue tracking', function (): void {
    $cache = app('cache');
    $metrics = new AnalyticsMetrics;
    $service = new AnalyticsDataService($cache, $metrics);
...
test('v5.9.0: AnalyticsDataService provider stats structure', function (): void {
    $cache = app('cache');
    $metrics = new AnalyticsMetrics;
    $service = new AnalyticsDataService($cache, $metrics);
...
test('v5.9.0: AnalyticsDataService funnel conversion', function (): void {
    $cache = app('cache');
    $metrics = new AnalyticsMetrics;
    $service = new AnalyticsDataService($cache, $metrics);
...
test('v5.9.0: EventTaxonomyService class exists with required methods', function (): void {
...
test('v5.9.0: EventTaxonomyService auto-classification', function (): void {
    $cache = app('cache');
...
test('v5.9.0: EventTaxonomyService tag groups', function (): void {
    $cache = app('cache');
    $service = new EventTaxonomyService($cache);
...
test('v5.9.0: EventTaxonomyService dynamic tags', function (): void {
    $cache = app('cache');
    $service = new EventTaxonomyService($cache);
...
test('v5.9.0: EventTaxonomyService multi-tag filtering', function (): void {
    $cache = app('cache');
    $service = new EventTaxonomyService($cache);
    $service->autoClassify();
...
test('v5.9.0: AnalyticsEventOccurred Laravel event exists', function (): void {
...
test('v5.9.0: TenantAnalyticsContext class exists with required methods', function (): void {
...
test('v5.9.0: TenantAnalyticsContext basic operations', function (): void {
    $cache = app('cache');
...
test('v5.9.0: TenantAnalyticsContext withinTenant scope', function (): void {
    $cache = app('cache');
    $context = new TenantAnalyticsContext($cache);
...
test('v5.9.0: config has new sections', function (): void {
...
test('v5.9.0: routes include dashboard, taxonomy, and tenant endpoints', function (): void {
...
test('v5.9.0: ServiceProvider registers new services', function (): void {
...
test('v5.9.0: Inertia middleware still implements HttpMiddlewareContract', function (): void {
    $ref = new ReflectionClass(HandleInertiaAnalytics::class);
    expect($ref->implementsInterface(HttpMiddlewareContract::class))->toBeTrue();
});

test('v5.9.0: event catalog still validates cleanly', function (): void {
...
test('v5.9.0: services layer has 85+ files', function (): void {
...
test('v5.9.0: test file count is 165+', function (): void {
    $testDir = __DIR__;
