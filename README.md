...
||[![Latest Version](https://img.shields.io/badge/version-151.0.0-blue)](https://github.com/zeroboiler/analytics)||
...
### What's New in v151.0.0

**Event Catalog Typed Factory Methods — One-Line Typed Event Builders**:

- **54 typed shorthand factory methods** across all 3 primary event catalogs (Ecommerce, SaaS, Engagement). Each catalog now provides one-line typed event builders that return ready-to-dispatch `AnalyticsEvent` DTOs with the correct category pre-set.
- **EcommerceEvents** — 15 specific shorthand methods: `viewItem()`, `addToCart()`, `removeFromCart()`, `viewCart()`, `beginCheckout()`, `addPaymentInfo()`, `purchase()`, `refund()`, `addToWishlist()`, `selectItem()`, `selectPromotion()`, `viewPromotion()`, `checkoutStep()`, `abandonedCart()`, `checkoutAbandon()` + generic `build()`.
- **SaaSEvents** — 21 specific shorthand methods: `signUp()`, `login()`, `logout()`, `startTrial()`, `subscribe()`, `planUpgrade()`, `planDowngrade()`, `cancellation()`, `featureUsed()`, `revenueTracked()`, `subscriptionCreated()`, `subscriptionCancelled()`, `trialConverted()`, `trialExpired()`, `inviteAccepted()`, `workspaceCreated()`, `firstValue()`, `activation()`, `paymentFailed()`, `paymentSucceeded()` + generic `build()`.
- **EngagementEvents** — 15 specific shorthand methods: `pageView()`, `scrollDepth()`, `click()`, `formStart()`, `formSubmit()`, `search()`, `share()`, `error()`, `jsError()`, `sessionStart()`, `sessionEnd()`, `feedback()`, `consentGranted()`, `consentWithdrawn()`, `onboardingCompleted()` + generic `build()`.
- **Generic `build()` factory** on each catalog — validates event name against the catalog and returns a typed event with correct category, throwing `InvalidArgumentException` for unknown events.
- **Type-safe params** — Key shorthand methods use typed parameters (e.g., `planUpgrade(string $fromPlan, string $toPlan)`, `scrollDepth(int $percent)`, `revenueTracked(float $amount, string $currency)`) with proper PHPDoc type hints.
- **V151EventCatalogTypedFactoryTest** — 75+ assertion test covering all 3 catalogs: typed return values, category correctness, parameter merging, exception handling for invalid names, cross-catalog consistency, and serialization readiness for queue dispatch.
- **Version sweep** — All 14 entry points synced to v151.0.0 (composer.json, package.json, analytics.js, analytics.d.ts, analytics.constants.js, 7 Svelte composables, AnalyticsEvent::VERSION, AnalyticsIntegrityCommand::EXPECTED_VERSION, README badge, 2 service @since tags).
- **Zero breaking changes** — All new factory methods are additive. Existing dispatch pipeline, catalog structure, and provider implementations unchanged.
...
**Industry-Standard SaaS Analytics Starter — Full Feature Audit (Phase 39)**:

- **`Phase39SaaSIndustryStandardAuditTest`** — 100+ assertion comprehensive audit validating all 12 planned SaaS analytics features are implemented and production-ready. Covers Event Catalog (210+ typed events), Server-Side Lifecycle Tracker, Inertia middleware, API controller (200+ routes), JS client (~8200 LOC), Event queue, User identity linking, E-commerce helpers, Admin commands (75+), Config expansion, Optional providers (Plausible, PostHog — 10 total), Tests (200+) + README (8600+ LOC).
- **Verified production-ready** — All 12 features confirmed at industry-standard SaaS starter level. Package provides 10 provider trackers, 210+ typed events across 8 categories, 322+ services, 75+ artisan commands, full consent/GDPR pipeline, revenue attribution, cohort analytics, identity resolution, and comprehensive TypeScript definitions.
- **Version sweep** — All version entry points synced to v147.0.0 across 25+ files.
- **Zero breaking changes** — Audit test is additive. Existing dispatch pipeline unchanged.
...
- **`EventSchemaOpenApiGenerator`** — Generates machine-readable OpenAPI 3.0.3 specification from the analytics event catalog and API routes. Covers all 35+ API endpoint definitions with request/response schemas, security schemes (Sanctum Bearer + SDK Token), tag-based grouping (23 endpoint categories), and configurable metadata (title, description, contact, license). Supports both JSON and YAML export formats.
- **JSON export** — `GET /api/analytics/openapi-spec` — Returns the full OpenAPI spec as JSON. Compatible with Swagger UI, Redoc, and any OpenAPI 3.0 tooling.
- **YAML export** — `GET /api/analytics/openapi.yaml` — Returns the full OpenAPI spec in YAML format. Direct import into API gateways, documentation generators, and CI/CD pipelines.
- **Endpoint summary** — `GET /api/analytics/openapi/endpoints` — Flat list of all API endpoints with HTTP method, path, description, and tag groupings. Quick reference for developers and dashboards.
- **New config section** — `zeroboiler.analytics.openapi` with `enabled`, `title`, `description`, `version`, `contact`, and `license` settings. Metadata is customizable to match your application's documentation standards.
- **15 test cases** — Comprehensive coverage for OpenAPI spec structure, info section customization, all core endpoints (events, batch, identify, consent, health, catalog, schemas, identity, GDPR, openapi), request body schema validation, security scheme definitions, error response schemas, JSON output validation, YAML output validation, endpoint summary format, tag coverage, response code completeness, and specific request schema validations (identify, consent, delete).
- **Version sweep** — All package files synced to 127.0.0 (composer.json, package.json, AnalyticsEvent::VERSION, AnalyticsServiceProvider, JS client, TypeScript definitions).

### What's New in v126.0.0

**Phase 126 — Unified Category Dispatchers & Event Field Validation Integration**:

- **`Analytics::trackEcommerceEvent()`** — Category-scoped ecommerce dispatcher. Accepts event name + params, validates against EcommerceEvents catalog, injects currency/value/transaction_id from options, and dispatches to all providers. Returns true if event was found in catalog, false otherwise.
- **`Analytics::trackSaaSLifecycle()`** — Category-scoped SaaS lifecycle dispatcher. Validates against SaaSEvents catalog and dispatches with category context.
- **`Analytics::trackEngagement()`** — Category-scoped engagement dispatcher. Validates against EngagementEvents catalog.
- **`Analytics::trackByCategory()`** — Universal category-aware dispatcher. Auto-detects category from EventCatalog::getCategory() and routes to the appropriate category dispatcher. Fallback to generic `track()` for unregistered events.
- **AnalyticsFake parity** — All 4 new dispatchers have no-op implementations in AnalyticsFake (returns true) for seamless test environment usage.
- **Facade docblock** — All 4 new methods documented in the Analytics facade with full parameter types and return types.
- **JS client parity** — `trackEcommerceEvent()`, `trackSaaSLifecycle()`, `trackEngagement()`, `trackByCategory()` exported from analytics.js with catalog validation.
